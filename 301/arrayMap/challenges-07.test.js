...
const charCode = (arr) => {
  // Solution code here...
  let arrCode = arr;
  return arrCode.map(charElement => charElement.charCodeAt(0));
...
const extractStats = (arr) => {
  // Solution code here...
  return arr.map(iter => {
    return {name: iter.stat.name, total: (iter.effort + iter.baseStat)};
  })
};
